...
- Never pass `secrets.json`, `.env`, or full `config.json` contents (including DB connection strings) in `task` or `extraContext`.
- Prefer `electron/`, `src/`, and `mcp-server/` paths for reviews; do not include end-user photo library paths unless the user `@`-mentions them.
- External review complements `/pr-ready` — it does not replace lint, `tsc`, or Vitest.
