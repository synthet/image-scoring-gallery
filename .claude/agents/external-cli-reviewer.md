...
description: Coordinate review-only Codex and Gemini runs through subagent-orchestrator MCP for image-scoring-gallery, including sequential panel-style comparisons.
---
...
- Use only the `subagent-orchestrator` MCP server for external CLI agents (project key **`imgscore-el-subagent-orchestrator`**).
